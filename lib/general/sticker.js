...
async function writeExifImg(media, metadata) {
  ...
  if (metadata.packname || metadata.author) {
    ...
    const json = {
      "sticker-pack-id": `https://github.com/Jabalsurya2105`,
...
async function sticker5(img, url, packname, author, categories = ['']) {
  try {
    let imageBuffer;
    if (img) {
      imageBuffer = Buffer.isBuffer(img) ? img : require('fs').readFileSync(img);
    } else if (url) {
      ...
      throw new Error('Harus menyediakan img atau url');
    }
    ...
    const response = await axios.post('https://api.sticker-service.com/create', formData, {
      headers: {
        ...formData.getHeaders(),
      },
    });
    if (response.status === 200 && response.data.success) {
      const stickerUrl = response.data.stickerUrl;
      const stickerResponse = await axios.get(stickerUrl, {
        responseType: 'arraybuffer'
      });
      return Buffer.from(stickerResponse.data);
    } else {
      throw new Error(`Gagal membuat stiker: ${response.data.message || 'Unknown error'}`);
    }
  } catch (error) {
