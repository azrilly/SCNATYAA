...
async function resize(buffer, width, height) {
    const inputPath = path.resolve(__dirname, `input_${Date.now()}.jpg`)
    const outputPath = path.resolve(__dirname, `output_${Date.now()}.jpg`)
        ...
        try {
            ...
            exec(command, (error) => {
                ...
                try {
                    ...
                    reject(new Error(`Error reading: ${readError.message}`))
                }
            })
        } catch (writeError) {
            if (fs.existsSync(inputPath)) fs.unlinkSync(inputPath)
            reject(new Error(`Error writing: ${writeError.message}`))
        }
    })
}
