...
const jadibot = async (Lyrra, m, from) => {
  if (Object.keys(client).includes(from)) {
    return Lyrra.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await Lyrra.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../../Lyrra')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(Lyrra, m, from) {
  if (!Object.keys(client).includes(from)) {
    return Lyrra.sendMessage(from, {
...
async function listjadibot(Lyrra, m) {
