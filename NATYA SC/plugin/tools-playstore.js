...
let handler = async (m, { Lyrra, text, command }) => {
    if (!text) return m.reply(`Contoh: ${command} whatsapp`)
    try {
        let loo = await fetchJson(`https://api.vreden.web.id/api/playstore?query=${encodeURIComponent(text)}`)
        ...
        await Lyrra.sendMessage(m.chat, { image: { url: yoo.img }, caption: tekz }, { quoted: m })
    } catch (err) {
        console.error(err)
        m.reply(err)
    }
